 ...
 */
export function moveaxis<T extends DType>(
  a: NDArray<T>,
  source: number,
  destination: number,
): NDArray<T> {
...
export function deleteArr<T extends DType>(
  ...
  if (axis === undefined) {
    // Flatten and delete
    const deleteSet = new Set(indicesArray.map((idx) => (idx < 0 ? data.buffer.length + idx : idx)))
...
export function insert<T extends DType>(
  ...
  if (axis === undefined) {
    ...
    for (const idx of indicesArray) {
      ...
      const insertValues = insertMap.get(normalizedIdx)
      if (!insertValues) {
        throw new Error('Internal insert index initialization failed')
      }
      insertValues.push(...valuesArray)
    }

    let destIdx = 0
    for (let i = 0; i <= data.buffer.length; i++) {
      const insertValues = insertMap.get(i)
      if (insertValues) {
