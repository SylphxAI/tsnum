...
import { array, cos, dot, exp, fft, ifft, log, matmul, sin } from '../src/index'
...
// Medium matrices
const a10x10 = array(
  Array.from({ length: 10 }, (_, i) => Array.from({ length: 10 }, (_, j) => i * 10 + j + 1)),
)
const b10x10 = array(
  Array.from({ length: 10 }, (_, i) => Array.from({ length: 10 }, (_, j) => (i + 1) * (j + 1))),
)
